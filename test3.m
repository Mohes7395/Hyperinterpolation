...
%% =========================================================================
%  TEST 3   [Lebesgue computation corrected]
%
...
for qi = 1:nq
    ...
    % SVD-consistent Lebesgue (same definition as kernel methods)
...
for k = 1:nk
    ...
    for idx = 1:nn
        ...
        h_X            = fill_distance(x_ctrs, 10000);
        ep             = C_ep * h_X;
        ep_used(k,idx) = ep;
...
for k = 1:nk
    ...
    lg = legend(sprintf('Poly Hyper (q=1:3:%d)', q_max),'Hyper Full','Hyper Reduced','Location','northwest');
    pub_legend(lg);
    title(sprintf('f_3 - Condition Number - %s - ep=%.1f*h_X - %s points', ...
...
for k = 1:nk
    ...
    pub_legend(lg);
    title(sprintf('f_3 - RMSE - %s  \\tau=%d, ep=%.1f*h_X - %s points', ...
...
for k = 1:nk
    ...
    pub_legend(lg);
    title(sprintf('f_3 - Lebesgue Constant - %s - ep=%.1f*h_X - %s points', ...
        rbf_names(k), C_ep_vec(k), data_distribution),'FontSize',12,'FontWeight','bold','Color','k');

    fprintf('\n[Lebesgue slopes]  %s | C_ep=%.2f\n', rbf_names(k), C_ep_vec(k));
    fprintf('  %-48s  %+.4f\n', 'Poly Hyper:',   slope_lpoly);
